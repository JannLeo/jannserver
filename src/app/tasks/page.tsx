        ...
        <div className="space-y-2">
          {tasks.map(t => (
            <div key={t.id} className="bg-white rounded-lg px-4 py-3 border border-slate-100 flex items-center gap-3">
              ...
              <span className="text-xs text-slate-400">{t.priority === 1 ? '🔥' : t.priority === 2 ? '📌' : '💤'}</span>
              {t.projectId && <span className="text-xs text-blue-500 bg-blue-50 px-2 py-0.5 rounded">{t.projectId}</span>}
